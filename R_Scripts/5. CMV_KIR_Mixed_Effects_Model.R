...
############################################ Global Variables #######################################################
setwd("/home/akshay-iyer/NK_Manuscript")
in.path <-"/home/akshay-iyer/NK_Manuscript/Saved_R_Data/"
...
######## Read in CMV/KIR data

# Read in the Excel file
CMV_KIR <- read_excel('/home/akshay-iyer/NK_Manuscript/CMV_KIR.xlsx')

tara_Freq <- tara_Freq %>%
  left_join(
    CMV_KIR %>%
      rename(PID = `Donor ID`),   # match column name
    by = c("PID", "Timepoint")    # match on both
  )


tara_Freq <- tara_Freq %>%
  filter(!is.na(`CMV IgG`), !is.na(Haplotype))

tara_Freq$Haplotype <- as.factor(tara_Freq$Haplotype)
...
flow_population_columns <- colnames(TARA_HUT78_Freq)[15:ncol(TARA_HUT78_Freq)-2] 
...
setwd("/home/akshay-iyer/NK_Manuscript/Mixed_Effects_Model/TARA/CMV_KIR/HUT78")

### Specific Killing
fixed_part_of_formula <- "`Specific Killing` ~ `viral load` + Timepoint  + gender + `CMV IgG` + Haplotype +  HIV + (1 | PID)"
...
setwd("/home/akshay-iyer/NK_Manuscript/Mixed_Effects_Model/TARA/CMV_KIR/K562")
...
setwd("/home/akshay-iyer/NK_Manuscript/Mixed_Effects_Model/TARA/CMV_KIR/Untreated/K562_Specific_Killing")
...
setwd("/home/akshay-iyer/NK_Manuscript/Mixed_Effects_Model/TARA/CMV_KIR/Untreated/HUT78_Specific_Killing")
