...
############################################ Global Variables #######################################################
setwd("C:/Users/axi313/Documents/NK_Manuscript/FCS")
in.path <-"C:/Users/axi313/Documents/NK_Manuscript/FCS/"
fcs_path <-"C:/Users/axi313/Documents/NK_Manuscript/FCS/All"

##### Read in flow files ###
F_metadata <- fread(paste0(in.path,"F_Metadata.csv"))
F_metadata[, File_Name := gsub("\\.fcs$", "_Total_NK.fcs", File_Name)]

# Get the list of FCS files that correspond to the metadata
files_to_load <- file.path(fcs_path, F_metadata$File_Name)


# Ensure the row names of F_metadata match the sample names in the flowSet

F_metadata <- as.data.frame(F_metadata)
rownames(F_metadata) <- F_metadata$File_Name
...
fcs_files_corrected <- lapply(F_metadata$File_Name, function(file_name) {
...
names(fcs_files_corrected) <- F_metadata$File_Name

###########
sapply(fcs_files_corrected, function(flowFrame) nrow(exprs(flowFrame)))

#Specter Pipeline

zero_row_indices <- which(sapply(fcs_files_corrected, function(flowFrame) nrow(exprs(flowFrame)) == 0))

# List the problematic flowFrames
zero_row_files <- sapply(zero_row_indices, function(idx) identifier(fcs_files_corrected[[idx]]))
zero_row_files ## "export_G4_AI-1180_NK+CEM_Total_NK.fcs" "export_H4_AI-1313_NK+CEM_Total_NK.fcs" 

### Remove 0 row files
fcs_files_corrected <- fcs_files_corrected[-zero_row_indices]
...
# Add additional metadata from F_metadata if necessary (e.g., Batch, Treatment)
# Ensure that the column 'SampleID' in combined_data matches with 'File_Name' in F_metadata
combined_data <- merge(combined_data, F_metadata, by.x = "SampleID", by.y = "File_Name", all.x = TRUE)
...
setwd("C:/Users/axi313/Documents/NK_Manuscript/FLOWSOM/FLORAH/Results/QC")
...
plot.against <- "FJComp-BV605-A_asinh"
...
cellular.cols <- names(combined_data_norm)[c(42:67)]
as.matrix(cellular.cols)
cluster.cols <- names(combined_data_norm)[c(44:48,50:59,61,63:67)]
...
data.frame(table(combined_data_norm[[HIV.col]])) # Check number of cells per sample.
...
sub.targets <- c(5000,5000,5000,5000,5000)
...
combined_data_filtered_2 <- combined_data_filtered[!(combined_data_filtered$HIV_Status == 'HUU'), ]
...
make.colour.plot(batch.sub, "UMAP_X", "UMAP_Y", "FlowSOM_metacluster", col.type = 'factor', add.label = TRUE,
                 title = 'Pre Batch effect Correction - Flowsom (-HUU)',dot.size=0.5,plot.width	=13)

make.colour.plot(batch.sub, 'UMAP_X', 'UMAP_Y', 'Batch', 'factor', title = 'Pre Batch effect Correction - By Batch (-HUU)'
                 ,dot.size=0.5,plot.width	=13)
...
mod <- model.matrix(~ HIV_Status  + Condition, data = combined_data_filtered_f_2)
...
setwd("C:/Users/axi313/Documents/NK_Manuscript/FLOWSOM")
saveRDS(batch.corrected_filtered, file = "FLORAH_batch_corrected_filtered_FlowSOM.rds")
saveRDS(batch.corrected, file = "FLORAH_batch_corrected_data.rds")
